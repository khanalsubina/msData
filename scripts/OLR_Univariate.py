...
def main():
    ...
    metrics_file = results_dir / f"olr_uni_metrics.csv"
    metrics_df.to_csv(metrics_file, index=False)
    print("Saved metrics to:", metrics_file)


    # Plot test forecasts only
    ...
    plt.title(f"OLR Univariate\nRMSE: {rmse_scaled:.4f}, MAE: {mae_scaled:.4f}")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.close()
